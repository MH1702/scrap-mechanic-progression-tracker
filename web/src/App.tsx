...
export function App() {
  ...
  const visibleMarkers = useMemo(
    () => markers.filter((marker: PoiMarker) =>
      !hiddenMarkerKeys.has(marker.key),
    ),
    [hiddenMarkerKeys, markers],
  )
  ...
  const selectedMarker = markers.find((marker) => marker.key === selectedKey)
    ...
    <div className="grid h-full grid-rows-[auto_1fr_auto] bg-background text-foreground">
      <header className="z-30 flex items-center justify-between gap-4 border-b bg-card/95 px-4 py-3 shadow-sm backdrop-blur">
        <div className="min-w-0">
          <h1 className="font-heading text-xl font-semibold tracking-tight">Scrap Mechanic Progression Tracker</h1>
      ...
      <div className={`grid min-h-0 ${showRightSidebar ? "grid-cols-[19rem_minmax(0,1fr)_20rem]" : "grid-cols-[19rem_minmax(0,1fr)]"}`}>
        <MarkerSidebar
          model={reader.model}
          markers={markers}
      ...
      <footer className="z-30 border-t bg-card px-4 py-2 text-xs text-muted-foreground">
